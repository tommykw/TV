...
class VideoDetailsFragment : DetailsFragment() {
    ...
    protected fun updateBackground(uri: String) {
        Glide.with(activity).load(uri).centerCrop().error(mDefaultBackground).into(object : SimpleTarget<GlideDrawable>(mMetrics!!.widthPixels, mMetrics!!.heightPixels) {
    ...
    private fun setupDetailsOverviewRow() {
        ...
        Glide.with(activity).load(mSelectedMovie!!.cardImageUrl).centerCrop().error(R.drawable.default_background).into(object : SimpleTarget<GlideDrawable>(width, height) {
            override fun onResourceReady(resource: GlideDrawable,
                                         glideAnimation: GlideAnimation<in GlideDrawable>) {
